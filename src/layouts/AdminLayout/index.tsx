...
export default function AdminLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 text-gray-800 w-full text-left">
      {/* Sidebar */}
      <aside
        className={`hidden lg:flex flex-col shrink-0 bg-white transition-all duration-300 ease-in-out overflow-hidden ${isSidebarOpen ? 'w-64 border-r border-gray-200' : 'w-0 border-r-0'
          }`}
      >
        ...
        <header className="h-16 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/50 backdrop-blur flex items-center justify-between px-4 sm:px-8">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="hidden lg:flex p-1.5 rounded-lg border border-gray-200 hover:bg-gray-100 bg-transparent cursor-pointer items-center justify-center text-gray-600 transition-colors"
              title={isSidebarOpen ? "Đóng menu" : "Mở menu"}
            >
              {isSidebarOpen ? (
                <span className="material-symbols-outlined" style={{ fontSize: '24px' }}>left_panel_open</span>
              ) : (
                <span className="material-symbols-outlined" style={{ fontSize: '24px' }}>right_panel_close</span>
              )}
            </button>
