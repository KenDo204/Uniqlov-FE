...
const UpdateAddressModal = ({ open, setOpen, address }: UpdateAddressModalProps) => {
  const { updateAddress, fetchAddresses } = useAddress();
  const { 
    provinces, 
    districts, 
    wards, 
  ...
  }, [open, address, fetchProvinces, fetchDistricts, fetchWards, clearDistrictsWards]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.error(err || "Lỗi khi cập nhật địa chỉ");
    }
  };
