  ...
  return (
    <section className="space-y-6 max-w-[1400px] mx-auto px-4 lg:px-8">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-2xl md:text-3xl font-heading font-black m-0">New Arrivals</h2>
        </div>
        <button
        ...
        >
          Explore All New <ArrowRight className="w-3.5 h-3.5" />
        </button>
      </div>
      ...
      >
        {newArrivals.map((product) => {
          const activeColor = selectedColors[`new-${product.id}`] || product.variants[0]?.colorName || '';
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-1.5 pt-1.5" onClick={(e) => e.stopPropagation()}>
                      {product.variants.map((v) => (
                        <button
                          key={v.id}
                          onClick={() => setSelectedColors((prev) => ({ ...prev, [`new-${product.id}`]: v.colorName }))}
